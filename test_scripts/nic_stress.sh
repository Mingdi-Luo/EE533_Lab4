#!/bin/bash
# ------------------------------------------------------------
# Step 6 (NIC mode): UDP small-packet stress test
#
# Test characteristics:
#   - Full node-pair coverage (n0..n3)
#   - 3 rounds of perfect matchings
...
# Output files
MASTER_SUMMARY="udp_nic_summary.log"
LOCAL_LOG_DIR="./udp_nic_logs"
...
cleanup_iperf() {
  echo
  echo "[*] Cleaning up iperf servers and remote UDP logs..."
  for H in "${HOSTS[@]}"; do
    sshpass -p "$PASS" ssh -o StrictHostKeyChecking=no \
      "$USER@$H" \
      "pkill -u $USER iperf 2>/dev/null; rm -f /tmp/udp_nic_*.log 2>/dev/null"
  done
...
# ------------------------------------------------------------
echo "--- Step 6 UDP NIC Stress Test ($(date)) ---" > "$MASTER_SUMMARY"
...
# ------------------------------------------------------------
# Execute 3 rounds of UDP stress tests
# ------------------------------------------------------------
for R in 0 1 2; do
  ROUND_ID=$((R+1))
  PAIRS=(${ROUNDS[$R]})
  ...
  echo
  echo "=== UDP NIC Round ${ROUND_ID} ==="

  # Start one UDP iperf server on each node with a fixed port
  echo "[*] Starting UDP servers on ports 5160-5163..."
  for i in 0 1 2 3; do
    H=${HOSTS[$i]}
    P=${PORTS[$i]}
    sshpass -p "$PASS" ssh -o StrictHostKeyChecking=no \
  ...
  # Launch all UDP clients concurrently
  echo "[*] Launching concurrent UDP flows..."
  for ((i=0; i<${#PAIRS[@]}; i+=2)); do
    ...
    REMOTE_LOG="/tmp/udp_nic_${SRC_NAME}_to_${DST_NAME}_r${ROUND_ID}.log"

    echo "  [FLOW] ${SRC_NAME} -> ${DST_NAME} (dst_port=${DST_PORT})"
...
# ------------------------------------------------------------
echo
echo "[*] Fetching remote UDP logs..."
for i in 0 1 2 3; do
  SRC_HOST=${HOSTS[$i]}
  sshpass -p "$PASS" scp -o StrictHostKeyChecking=no \
    "$USER@$SRC_HOST:/tmp/udp_nic_*.log" \
...
for R in 0 1 2; do
  ...
  for ((i=0; i<${#PAIRS[@]}; i+=2)); do
    ...
    SRC_NAME=${NAMES[$SRC]}
    DST_NAME=${NAMES[$DST]}
    FILE="$LOCAL_LOG_DIR/udp_nic_${SRC_NAME}_to_${DST_NAME}_r${ROUND_ID}.log"
...
# ------------------------------------------------------------
echo
echo "=== Final UDP NIC Summary ==="
cat "$MASTER_SUMMARY"

echo
echo "[*] Step 6 NIC-mode UDP test completed."
